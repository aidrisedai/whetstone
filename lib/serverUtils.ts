 ...
 * Throws with a descriptive message if the extracted text is not valid JSON.
 */
export function safeParseJson<T>(text: string): T {
  ...
  try {
    ...
    throw new Error(`Failed to parse model response as JSON. Raw text: ${text.slice(0, 200)}`);
  }
}
