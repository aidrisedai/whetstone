...
const CONFETTI_PIECES = Array.from({ length: 26 }, (_, idx) => ({
...
function Confetti() {
  return (
    <div className="pointer-events-none absolute inset-0 z-30 overflow-hidden">
      {CONFETTI_PIECES.map(({ left, delay, dur, size, idx }) => (
        <span
          key={idx}
          style={{
            position: "absolute",
            ...
            width: size,
            height: size,
            background: CONFETTI_COLORS[idx % CONFETTI_COLORS.length],
            borderRadius: idx % 2 ? "50%" : "2px",
