...
export function CodeLesson({
  ...
  // Pre-compute per-beat starting line offsets to avoid mutation inside map.
  const lineOffsets: number[] = [];
  { let acc = 0; for (const b of beats.slice(0, i + 1)) { lineOffsets.push(acc); acc += b.code.split("\n").length; } }
  return (
