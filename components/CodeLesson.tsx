...
export function CodeLesson({
  ...
  // Precompute cumulative line offsets so render is pure (no mutation during map).
  const lineOffsets = useMemo(() => {
    const offsets: number[] = [];
    let offset = 0;
    beats.slice(0, i + 1).forEach((b) => {
      offsets.push(offset);
      offset += b.code.split("\n").length;
    });
    return offsets;
            ...
            {onIntro ? (
              ...
              <div className="font-mono text-[12.5px] leading-[1.65]">
                {beats.slice(0, i + 1).map((b, idx) => {
                  ...
                  const beatOffset = lineOffsets[idx] ?? 0;
                  return (
                    <div
                      key={idx}
