...
export function CodeLesson({
  ...
  // Pre-compute per-beat starting line numbers (1-based) so the renderer never
  // mutates a variable inside JSX.
  const renderedBeats = beats.slice(0, i + 1);
  const beatLineStarts: number[] = [];
  {
    let ln = 1;
    for (const b of renderedBeats) {
      beatLineStarts.push(ln);
      ln += b.code.split("\n").length;
    }
  }
          ...
          <div ref={codeScrollRef} className="h-[62vh] overflow-auto py-3">
            {onIntro ? (
              ...
              <div className="font-mono text-[12.5px] leading-[1.65]">
                {renderedBeats.map((b, idx) => {
                  const active = idx === i;
                  const lines = b.code.split("\n");
                  const beatStart = beatLineStarts[idx];
                  return (
                    <div
                      key={idx}
