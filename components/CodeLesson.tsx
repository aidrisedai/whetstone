...
export function CodeLesson({
  ...
  // Pre-compute the starting line number for each beat (1-based, continuous).
  const beatStartLines = useMemo(
    () =>
      beats.reduce<number[]>((acc, b, idx) => {
        acc.push(idx === 0 ? 1 : acc[idx - 1] + beats[idx - 1].code.split("\n").length);
        return acc;
      }, []),
    [beats],
  );
