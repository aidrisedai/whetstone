...
export function CodeLesson({
  ...
  // Pre-compute each beat's starting line number so line counters stay stable across re-renders.
  const beatStartLine = useMemo(() => {
    const lineCounts = beats.map((b) => b.code.split("\n").length);
    return lineCounts.map((_, i) => lineCounts.slice(0, i).reduce((s, c) => s + c, 0));
  }, [beats]);
                ...
                {beats.slice(0, i + 1).map((b, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {lines.map((ln, li) => {
                        const n = beatStartLine[idx] + li + 1;
                        let html = tint(ln);
