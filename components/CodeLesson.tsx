...
export function CodeLesson({
  ...
  const displayAskText = mic.listening ? mic.transcript : askText;
            ...
            {onIntro ? (
              ...
              <div className="font-mono text-[12.5px] leading-[1.65]">
                {beats.slice(0, i + 1).map((b, idx) => {
                  const lineOffset = beats.slice(0, idx).reduce((sum, prev) => sum + prev.code.split("\n").length, 0);
                  const active = idx === i;
          ...
          <div className="rounded-2xl border border-steel/30 bg-steel/5 p-2">
            ...
            <div className="flex items-end gap-2">
              ...
              {mic.supported && (
                <button
                  type="button"
                  onClick={() => (mic.listening ? mic.stop() : mic.start(displayAskText))}
              ...
              <button
                ...
                disabled={thinking || !displayAskText.trim()}
