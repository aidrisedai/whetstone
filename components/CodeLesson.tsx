...
export function CodeLesson({
  ...
  // Precompute per-beat line offsets so the line-number counter doesn't need
  // to mutate a variable during render.
  const lineOffsets = useMemo(
    () =>
      beats.slice(0, i + 1).reduce<number[]>(
        (acc, _b, idx) => [
          ...acc,
          idx === 0 ? 0 : acc[idx - 1] + beats[idx - 1].code.split("\n").length,
        ],
        [],
      ),
    [beats, i],
